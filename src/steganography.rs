...
impl Steganography {
  pub fn encrypt(image1 : DynamicImage, image2: DynamicImage) -> ImageBuffer<Rgb<u8>, Vec<u8>> {
    ...
    for x in 0..width_img1 {
        for y in 0..height_img1 {
            ...
            result_img_rgb.put_pixel(x, y, Steganography::merge_bit(*img1_rgb.get_pixel(x, y), rbg_converted));
        }
    }
    return result_img_rgb;
  }
 
  pub fn decrypt(image: DynamicImage, to_crop: bool)  -> (ImageBuffer<Rgb<u8>, Vec<u8>>, ImageBuffer<Rgb<u8>, Vec<u8>>) {
    ...
    for x in 0..width {
        for y in 0..height {
            let (rgb1, rgb2) = Steganography::split_pixel(*image_rbg.get_pixel(x, y));
  ...
  fn merge_bit(pixel1 : Rgb<u8>, pixel2: Rgb<u8>) -> Rgb<u8> {
    let r1 = Steganography::msb(pixel1[0]);
    let g1 = Steganography::msb(pixel1[1]);
    let b1 = Steganography::msb(pixel1[2]);

    let r2 = Steganography::msb_to_lsb(Steganography::msb(pixel2[0]));
    let g2 = Steganography::msb_to_lsb(Steganography::msb(pixel2[1]));
    let b2 = Steganography::msb_to_lsb(Steganography::msb(pixel2[2]));

    Rgb([r1 + r2, g1 + g2, b1 + b2])
  }

  fn split_pixel(pixel: Rgb<u8>) -> (Rgb<u8>, Rgb<u8>) {
    let r1 = Steganography::msb(pixel[0]);
    let g1 = Steganography::msb(pixel[1]);
    let b1 = Steganography::msb(pixel[2]);

    let r2 = Steganography::lsb_to_msb(Steganography::lsb(pixel[0]));
    let g2 = Steganography::lsb_to_msb(Steganography::lsb(pixel[1]));
    let b2 = Steganography::lsb_to_msb(Steganography::lsb(pixel[2]));
